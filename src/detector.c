  ...
  while(get_current_batch(net) < net.max_batches){
    ...
    */
    ...
    if ( (i==100) || (i % 500 == 0) ) {
      ...
      char buff_loss[256];
      sprintf(buff_loss, "%s/%s_%06d.loss", loss_directory, base, i);

      // Write training loss_directory...
      FILE *fh = fopen(buff_loss, "w");
      if (fh == NULL) {
	printf("Error opening file!\n");
	exit(1);
      }
      fprintf(fh, "%d: %f, %f avg, %f rate, %lf seconds, %d images\n",
...
void run_detector(int argc, char **argv)
{
  ...
  if(0==strcmp(argv[2], "test")) test_detector(datacfg, cfg, weights, filename, thresh, hier_thresh, dont_show, ext_output);
    ...
    char *name_list = option_find_str(options, "names", "data/names.list");
    int out_fps = find_int_arg(argc, argv, "-out_fps", 25);
    char **names = get_labels(name_list);
    if(filename)
      if(strlen(filename) > 0)
	if (filename[strlen(filename) - 1] == 0x0d) filename[strlen(filename) - 1] = 0;
    demo(cfg, weights, thresh, hier_thresh, cam_index, filename, names, classes,
	 frame_skip, prefix, out_filename, http_stream_port, dont_show, out_fps);
  }
